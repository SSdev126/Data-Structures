...
Graph newGraph(int n){
  ...
  G->L = calloc(sizeFixed , ( 4 + sizeof( List )));
  G->color = calloc(sizeFixed ,( 4 + sizeof( int )));
  G->parents = calloc(sizeFixed ,(4 + sizeof( int)));
  G->distance = calloc(sizeFixed , ( 4 + sizeof( int)));
  G->discovery = calloc(sizeFixed , ( 4 + sizeof( int)));
  G->finishTime = calloc(sizeFixed , ( 4 + sizeof( int)));
  G->order = n;
  G->size = 0;
  G->source = NIL;
...
int getParent(Graph G, int u){
  if( u <= getOrder(G) && u >= 1){
    if( G->parents[u] == 0 || G->parents[u] == NIL){
      return(NIL);
    }else{
      return(G->parents[u]);
    }
...
// returns the discovery time of the vertex u if DFS has ran else returns undefined
// Pre: 1 <= u <= n = getOrder(G)
int getDiscover(Graph G, int u){
  if( u < 1 || u > getOrder(G)){
    printf("Graph Error: input outside of graph range.\n");
    exit(EXIT_FAILURE);
  }
  if( G->discovery[u] == 0 ){
    return(UNDEF);
...
// returns the finish time of the vertex u if DFS has ran else returns undefined
// Pre: 1 <= u <= getOrder(G)
...
int visit( Graph G, List s, int time, int x){
  ...
  if( index(G->L[x]) != NIL){
	...
	if(G->color[y] == WHITE){
	  ...
	  time = visit( G, s, time, y);
	}
      }
    }
  }
  //printf("made it past visit loop %d\n", x);
  G->color[x] = BLACK;
  ++time;
  G->finishTime[x] = time;
  //printf("breaking in insert after %d \n", get(s));
  if( index(s) == NIL){
    prepend(s, x);
  }else{
  insertAfter(s, x);
...
// runs the DFS algorithm on Graph G and using List s to define the search
// order and function as a stack to store the vertices
// Pre: length(S) == getOrder(G)
void DFS(Graph G, List s){
  int i, x;
  int time = 0;
  if( length(s) != getOrder(G)){
    printf("Graph Error: DFS List length and Graph order are not equal.\n");
    exit(EXIT_FAILURE);
  }
  for( i = 1; i <= getOrder(G); i++){
    G->color[i] = WHITE;
...
Graph transpose(Graph G){
  ...
  for( i = 1; i <= getOrder(G); i++){
    if( length(G->L[i]) != 0){
      moveFront(G->L[i]);
      for( j = 1; j <= length(G->L[i]); j++){
	addArc( T, get(G->L[i]), i);
	moveNext(G->L[i]);
      }
    }
  }
...
void printGraph(FILE *out, Graph G){
  ...
  for( i = 1; i <= getOrder(G); i++){
    fprintf(out ,"%d: ", i);
    //printf(" %d has %d elements in it list\n", i, length(G->L[i]));
    if( length( G->L[i]) != 0){
